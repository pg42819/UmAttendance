package pt.uminho.pg42819.attendance.utils;
...
{
	private static final String LOGTAG = ResourceUtils.class.getSimpleName();
	...
	/**
	 * Load an list of courses from the builtin JSON file for courses
	 */
	public List<Course> loadCoursesFromLocalJson()
	{
		Gson gson = new GsonBuilder().create();
		...
		final List<Course> courses = gson.fromJson(jsonString, listType);
		for (Course course : courses) {
			Log.i(LOGTAG, course.toString());
		}
		return courses;
